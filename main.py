...
# 无论依赖是否齐全，都要定义类，这样AstrBot才能找到它
# 只是在运行时检查依赖，如果缺少依赖则给出错误提示
@register(
    "nimt_notice_monitor",
    "南京机电职业技术学院通知监控插件",
    "2.1.0",
...
class NJIMTNoticeMonitor(Star):
    def __init__(self, context: Context):
        ...
        if not HAS_DEPENDENCIES:
            logger.error("缺少必要的Python依赖：aiohttp, beautifulsoup4, rsa")
            logger.error("请安装依赖：pip install aiohttp beautifulsoup4 rsa")
            return

        if not HAS_ASTRBOT_API:
            logger.error("无法导入AstrBot API，请检查AstrBot版本")
            return

        # 初始化插件
        self.initialize_plugin(context)

    def initialize_plugin(self, context: Context):
        """初始化插件核心功能"""
        try:
...
-----END PUBLIC KEY-----"""
    ...
    def start_scheduler(self):
        ...
        try:
            ...
            # 课程检查任务
            @scheduler.scheduled_job('interval', seconds=self.config.get("course_check_interval", 3600), id='nimt_check_courses')
    ...
    async def fetch_page(self, url: str, method: str = "GET", data: Dict = None,
        ...
        """通用异步请求函数"""
        if not HAS_DEPENDENCIES:
            logger.error("缺少aiohttp依赖，无法进行网络请求")
            return ""
                ...
                for course_day in kbxx:
                    ...
                    for course in courses:
                        if course.get('kcmc') and course.get('kcmc') != '-':
                            if not day_has_course:
                                response += f"{weekdays[day_index % 7]}:\n"
                                day_has_course = True

                            response += f"  📚 {course.get('kcmc', '')}\n"
                            if course.get('teacher'):
                                response += f"    👨‍🏫 {course.get('teacher', '')}\n"
                            if course.get('classroom'):
                                response += f"    🏫 {course.get('classroom', '')}\n"
                            if course.get('kssj') and course.get('jssj'):
                                response += f"    ⏰ {course.get('kssj', '')}-{course.get('jssj', '')}\n"
                            response += "\n"

            if response == f"📅 第{current_week}周课表\n\n":
                response = f"📭 第{current_week}周暂无课程安排"
